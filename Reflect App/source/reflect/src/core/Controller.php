...
class Controller {
  ...
  private function getDynamicDocument() {
    ...
    if($config['underMaintenanceMode'] === 'true' && !empty($config['cmsUnderMaintenancePageSlug'])) {
    ...
    // Search page
    if((strpos($_SERVER['REQUEST_URI'], "/?searchTerms=") !== false || strpos($_SERVER['REQUEST_URI'], "?searchTerms=") !== false) && $config['underMaintenanceMode'] === 'false') {
  ...
  private function getAddonFiles($addonConfig) {
    ...
    if($config['useTranspiledJs'] === 'true') {
